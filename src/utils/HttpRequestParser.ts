...
export class HttpRequestParser implements RequestParser {
    ...
    public async parseHttpRequest(name?: string): Promise<HttpRequestModel> {
        ...
        // parse headers lines
        const headers = parseRequestHeaders(headersLines, <RequestHeaders>this.settings.defaultHeaders, requestLine.url);
    ...
    private parseRequestLine(line: string): { method: string, url: string } {
        ...
        const matchValidMethod = /^(GET|POST|PUT|DELETE|PATCH|HEAD|OPTIONS|CONNECT|TRACE)\s+/i.exec(line);
        if (matchValidMethod) {
            method = matchValidMethod[1];
            url = line.substr(matchValidMethod[0].length);
        } else {
        ...
        url = url.trim();
        const matchValidProtocol = /\s+HTTP\/.*$/i.exec(url)
        if (matchValidProtocol) {
            url = url.substr(0, matchValidProtocol.index);
        }
